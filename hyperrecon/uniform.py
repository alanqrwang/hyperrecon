...
class UniformHypernetVariabilityPrior(BaseTrain):
  """UniformHypernetVariabilityPrior."""

  def __init__(self, args):
    super(UniformHypernetVariabilityPrior, self).__init__(args=args)
  ...
  def inference(self, zf, coeffs):
    return self.network(zf, coeffs), self.network.get_hyp_out(coeffs)
  ...
  def train_step(self, batch):
    ...
    with torch.set_grad_enabled(True):
      hparams = self.sample_hparams(batch_size)
      coeffs = self.generate_coefficients(hparams)
      pred, hyp_out = self.inference(zf, coeffs)

      loss, recon_loss, div_loss = self.compute_loss(pred, gt, y, coeffs, hyp_out, is_training=True)
      loss = self.process_loss(loss)
      loss.backward()
      self.optimizer.step()
    psnr = bpsnr(gt, pred)
  ...
  def compute_loss(self, pred, gt, y, coeffs, hyp_out=None, is_training=False):
    assert len(self.losses) == coeffs.shape[1], 'loss and coeff mismatch'
    recon_loss = 0
    for i in range(len(self.losses)):
      l = self.losses[i]
      if is_training:
        c = coeffs[:self.batch_size, i]
        recon_loss += c * l(pred[:self.batch_size], gt[:self.batch_size], y[:self.batch_size], None)
      else:
        c = coeffs[:, i]
        recon_loss += c * l(pred, gt, y, None)
    
    if is_training:
      # TODO: generalize to higher-order coefficients
      hparams = coeffs[:, 1]
      lmbda = torch.abs(hparams[:self.batch_size] - hparams[self.batch_size:])
      diversity_loss = (hyp_out[:self.batch_size] - hyp_out[self.batch_size:]).norm(p=2, dim=1)
      return recon_loss - self.beta*lmbda*diversity_loss, recon_loss, diversity_loss
    else:
      return recon_loss