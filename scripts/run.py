...
class Parser(argparse.ArgumentParser):
    def __init__(self):
        ...
        utils.add_bool_arg(self, 'preprocess_dataset', default=True)
...
def train(network, dataloader, criterion, optimizer, hpsampler, args):
    ...
    for batch_idx, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
        if not args.preprocess_dataset and batch_idx > 25: # Artificially reduce per-epoch training time
            break
...
def validate(network, dataloader, criterion, hpsampler, args):
    ...
    for batch_idx, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
        if not args.preprocess_dataset and batch_idx > 5: # Artificially reduce per-epoch training time
            break
        ...
        with torch.set_grad_enabled(False):
            ...
            recon, cap_reg = network(zf, hyperparams)
            loss, _, _ = criterion(recon, y, hyperparams, cap_reg, gt=gt)
...
if __name__ == "__main__":
    ...
    if args.preprocess_dataset:
        xdata = dataset.get_train_data(args.img_dims, args.undersampling_rate, organ=args.organ)
        gt_data = dataset.get_train_gt(args.img_dims, organ=args.organ)
